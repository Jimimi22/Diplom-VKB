...
class Template {
	...
	public function __construct(Config $config, ControllerInterface $controller) {
	...
	public function run($helper, $args) {
        if ($helper) {
			if (!array_key_exists($helper, $this->_helpers)) {
				$filepath = rtrim($this->_config->helpers, '/').'/'.$helper.'.hlp.php';
				if (file_exists($filepath)) {
                    ...
                    if (!$class->isSubclassOf('AbstractTemplateHelper')) {
